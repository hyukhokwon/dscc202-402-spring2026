...
slow_join_df = (transactions_df
    ...
    .filter(col("country") == "US") # Modify USA to US
)
...
duplicates_df = (base_customers_df
    ...
    .withColumn("last_name", when(col("duplicate_id") % 2 == 0, upper(col("last_name"))).otherwise(col("last_name"))
    ...
    .withColumn("email_address", when(col("duplicate_id") % 4 == 0, upper(col("email_address"))).otherwise(col("email_address"))
...
standardized_df = (dups_df
    ...
    # Normalize postal codes (remove spaces)
    .withColumn("postal_zip_code", regexp_replace(trim(col("postal_zip_code")), "[^0-9]", ""))
    # Normalize gender (remove spaces)
...
print("usa franchises:", usa_franchises_df.count())

print("after customer join:",
      transactions_df.join(clean_customers_df, "customerID").count())

print("after both joins:",
      transactions_df.join(clean_customers_df, "customerID")
