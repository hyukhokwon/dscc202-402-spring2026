...
# MAGIC ON target.tpep_pickup_datetime = source.tpep_pickup_datetime AND target.tpep_dropoff_datetime = source.tpep_dropoff_datetime AND target.pickup_zip = source.pickup_zip
...
silver_stream_df = (spark.readStream
    ...
    .withWatermark( "tpep_pickup_datetime", "1 hours" )  # Column and watermark interval
    .groupBy(
        window(col( "tpep_pickup_datetime"), "1 hours" ),  # Column and window duration
...
def upsert_to_gold(batch_df, batch_id):
    ...
    spark.sql("""
        ...
        ON target.tpep_pickup_datetime = source.tpep_pickup_datetime AND
