...
prediction_stats = predictions_df.select(
    mean(col("absolute_error")).alias("mean_absolute_error"),  # Mean absolute error
    stddev(col("absolute_error")).alias("stddev_absolute_error"),  # Std of absolute error
    mean(col("percentage_error")).alias("mean_percentage_error"),  # Mean percentage error
    percentile_approx(col("absolute_error"), 0.5).alias("median_absolute_error"),  # Median absolute error
    percentile_approx(col("absolute_error"), 0.95).alias("p95_absolute_error"),  # 95th percentile
    count(col("*")).alias("total_predictions")  # Total predictions
).collect()[0]
